...
class MetricRunner:
    ...
    def _calculate_mae_for_dataset(self, samples, properties):
        ...
        for sample in samples:
            ...
            if not isinstance(predicted_props, dict) or not isinstance(target_props, dict):
                continue
            ...
            for prop_key in properties:
                ...
                if predicted_val is not None and target_val is not None:
                    try:
                        all_prop_pairs[prop_key]['predicted'].append(float(predicted_val))
    ...
    def _process_dataset(self, samples, name, metrics, property_list): 
        ...
        # Process pre-computed properties
        if any(props_dicts):
        ...
        if token_based_metrics:
            ...
            if 'token_frequency' in metrics: results['token_frequency'] = token_counts
            if 'length_distribution' in metrics: results['length_distribution'] = lengths

        # Process chemical metrics
        ...
        chemical_metrics_list = [m for m in metrics if m not in ('validity', 'uniqueness', 'novelty', 'token_frequency', 'length_distribution') and m not in property_list]
    ...
    def run_multi(self, sample_dict, metrics, properties, reference_name, run_type):
        ...
        if not original_samples:
            logger.error("'Original data' not found in samples. Cannot compute novelty.")
            self.original_smiles = []
        else:
            orig_selfies, _ = _split_samples(original_samples)
            orig_mols = get_valid_molecules(orig_selfies, self.config)
            self.original_smiles = [Chem.MolToSmiles(m, canonical=True) for m in orig_mols if m]

        all_distribution_metrics = [m for m in (metrics + properties) if m not in ('validity', 'uniqueness', 'novelty', 'token_frequency', 'length_distribution')]
        ...
        for name, samples in sample_dict.items():
            ...
            is_conditioned_model = (
                run_type.startswith('conditioning') and
                name != self.reference_name and
                name != "Original data" and
                any(s.get("conditioning_properties") for s in samples if isinstance(s, dict))
            )
            if is_conditioned_model:
                logger.info(f"Calculating MAE for conditioned model: {name}")
                mae_distances[name] = self._calculate_mae_for_dataset(samples, all_distribution_metrics)
