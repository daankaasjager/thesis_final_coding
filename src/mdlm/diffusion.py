...
class Diffusion(L.LightningModule):
    ...
    def optimizer_step(self, *args, **kwargs):
        grad_norm = torch.nn.utils.clip_grad_norm_(
            self.parameters(), max_norm=1000000000.0
        )
        self.log("trainer/grad_norm", grad_norm, on_step=True, prog_bar=False)
    ...
    def q_xt(self, x, move_chance, attention_mask):
    ...
    def _sample_prior(self, *batch_dims, target_properties=None):
        ...
        *batch_dims : (int,) or (int, int)
        ...
        if self.config.model.sample_length_mode == "histogram":
            ...
            L = int(batch_dims[1])
        max_L = self.config.model.length
        if L > max_L:
            raise ValueError(
                f"Chosen length {L} exceeds model.length={max_L}. Increase the cap or trim the histogram."
            )
        ...
        return torch.full(
            (batch_size, L), self.mask_index, dtype=torch.long, device=self.device
        )
    ...
    def _ddpm_update(self, x, t, dt, target_properties, current_guidance_scale):
        ...
        q_xs_non_mask_term = torch.clamp(q_xs_non_mask_term, min=0)
    ...
    def restore_model_and_sample(
        self, num_steps, eps=1e-05, target_properties=None, guidance_scale=None
    ):
