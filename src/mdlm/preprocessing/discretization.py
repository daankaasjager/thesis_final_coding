...
def _bin_column(series: pd.Series, col_name: str, num_bins: int = 3):
    ...
    if num_bins <= 1:
        logger.warning(
            f"Number of bins must be >= 2 for column '{col_name}'. Skipping binning."
        )
    ...
    if not valid_idx.any():
        logger.warning(
            f"Column '{col_name}' contains no valid numeric data for binning."
        )
    ...
    try:
        ...
        logger.warning(
            f"qcut failed for '{col_name}' (reason: {e_qcut}). Falling back to pd.cut."
        )
        try:
...
def apply_discretization(config: DictConfig, df: pd.DataFrame):
    ...
    logger.info(
        f"Discretising {len(cols_to_bin)} columns into {num_bins} bins: {cols_to_bin}"
    )
    ...
    for col in tqdm(cols_to_bin, desc="Discretizing columns"):
        ...
        if bin_col_name in df_processed.columns:
            logger.warning(
                f"Bin column '{bin_col_name}' already exists. Skipping '{col}'."
            )
            continue
...
def map_target_properties_to_bins(
    config, target_properties: dict, tokenizer
) -> list[int]:
    """
