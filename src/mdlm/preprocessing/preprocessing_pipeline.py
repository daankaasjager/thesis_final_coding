...
def _tokenize_selfies_and_filter(
    ...
    for index, s in tqdm(df[selfies_col].items(), total=len(df), desc="Creating alphabet and filtering SELFIES"):
        ...
        try:
            ...
            logger.debug(f"Failed to read SELFIES string at index {index}: '{s}'. Error: {e}. Skipping.")
            error_count += 1
