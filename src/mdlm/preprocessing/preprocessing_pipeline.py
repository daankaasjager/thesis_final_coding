...
def _load_preprocessed_data(
    ...
    try:
        if preproc_path.suffix == ".parquet":
            ...
            logger.warning(
                f"Unsupported file format for preprocessed data: {preproc_path.suffix}. Expected .parquet."
            )
        ...
        logger.error(
            f"Could not load pre-processed data from {preproc_path} or alphabet from {alphabet_path}: {e!s}",
            exc_info=True,
        )
        raise
...
def _tokenize_selfies_and_filter(
    ...
    logger.info(
        f"Tokenizing '{selfies_col}' column and filtering sequences longer than {max_len} tokens..."
    )

    if pd.api.types.is_numeric_dtype(df[selfies_col]):
        logger.warning(
            f"Column '{selfies_col}' is numeric. Attempting to convert to string."
        )
    df[selfies_col] = (
        df[selfies_col].astype(str).fillna("")
    )  # convert NaNs to empty strings

    all_selfies = (
        df[selfies_col][df[selfies_col] != ""].unique().tolist()
    )  # Use unique for efficiency
    if not all_selfies:
        logger.warning("No valid non-empty SELFIES strings found to build alphabet.")
        alphabet = []
    else:
        logger.info(
            f"Building alphabet from {len(all_selfies)} unique non-empty SELFIES strings..."
        )
        try:
            ...
            logger.error(
                f"Failed to generate alphabet from SELFIES: {e}", exc_info=True
            )
            raise
    ...
    for index, s in tqdm(
        df[selfies_col].items(),
        total=len(df),
        desc="Creating alphabet and filtering SELFIES",
    ):
        ...
        try:
            ...
            logger.debug(
                f"Failed to read SELFIES string at index {index}: '{s}'. Error: {e}. Skipping."
            )
            error_count += 1
    ...
    if not valid_indices:
        ...
        return alphabet, pd.DataFrame(
            columns=df.columns.tolist() + ["tokenized_selfies"]
        )
    ...
    logger.info(
        f"Tokenization complete. Original rows: {original_rows}. Successfully tokenized & length-filtered: {final_rows}. Too long: {skipped_count}. Errors: {error_count}."
    )
    if original_rows > 0:
    ...
    if final_rows > 0:
        ...
        logger.warning(
            "No sequences remained after filtering. Check data and max_len setting."
        )
...
def prepare_data_for_training(config: DictConfig) -> Tuple[List[str], pd.DataFrame]:
    ...
    try:
        ...
        raw_data_path = Path(config.paths.original_data)
    except AttributeError as e:
        logger.error(f"Missing path configuration in `config.paths`: {e}")
        raise ValueError(f"Missing path configuration: {e}")

    if not config.checkpointing.fresh_data:
        logger.info(
            "Attempting to load existing pre-processed data (fresh_data=False)."
        )
        try:
            return _load_preprocessed_data(preproc_path, alphabet_path)
        except FileNotFoundError:
            logger.warning(
                f"Cached data not found ({preproc_path} or {alphabet_path}). Will recompute."
            )
        except Exception as e:
            logger.warning(
                f"Could not load cached pre-processed data: {e!s}. Will recompute."
            )

    logger.info(
        "Starting fresh data preprocessing pipeline (fresh_data=True or cache miss)..."
    )
    ...
    try:
        ...
        logger.error(
            f"Error during preprocessing pipeline (augmentation/discretization/tokenization): {e}",
            exc_info=True,
        )
        raise
    ...
    try:
        ...
        logger.error(
            f"Failed to save one or more preprocessing artifacts: {e}", exc_info=True
        )
        raise
