...
import re, json, pathlib, numpy as np, pandas as pd
TOKEN_PATTERN = re.compile(r"\[[^\]]+\]")

def build_histogram(data_path: str, column: str = "selfies") -> dict[int, int]:
    """Returns {length : count} for the whole corpus."""
    df = pd.read_csv(data_path)   # or read_csv
    # Fast token count: number of "[" in each string
...
if __name__ == "__main__":
    hist = build_histogram("/scratch/s3905845/thesis_final_coding/data/kraken/training_data/filtered_selfies.csv")
    save_histogram(hist, "/scratch/s3905845/thesis_final_coding/data/kraken/training_data/length_histogram.json")
