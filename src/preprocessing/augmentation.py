...
def apply_augmentation(df: pd.DataFrame, config: DictConfig) -> pd.DataFrame:
    ...
    logger.info(f"Starting data augmentation (target: {n_augmentations} versions per molecule)...")
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Augmenting molecules"):
        ...
        try:
             ...
             logger.warning(f"Could not parse SMILES '{original_smiles}': {e}. Skipping.")
             skipped_mols += 1
             continue 
        if mol is None:
            logger.warning(f"Skipping invalid or unparseable SMILES for augmentation: {original_smiles}")
            skipped_mols += 1
            continue
        ...
        while len(seen_smiles) < (n_augmentations + 1) and attempts < max_attempts_per_mol: 
            attempts += 1
            try:
                ...
                if rand_selfies is None:
                     logger.debug(f"SELFIES encoding failed for augmented SMILES: {rand_smiles}")
                ...
                logger.debug(f"Skipping one augmentation attempt for {original_smiles} due to error: {e}")
                continue

        if len(seen_smiles) < (n_augmentations + 1):
             logger.debug(f"Could only generate {len(seen_smiles)-1}/{n_augmentations} unique augmentations for {original_smiles} after {max_attempts_per_mol} attempts.")


    if not augmented_rows:
         logger.warning("Augmentation resulted in zero rows. Returning original DataFrame.")
         return df 

    augmented_df = pd.DataFrame(augmented_rows)
    logger.info(f"Data augmentation complete. Initial rows: {len(df)}, Augmented rows: {len(augmented_df)}. Skipped/invalid molecules: {skipped_mols}")
    ...
    if "selfies" not in augmented_df.columns and "selfies" in df.columns:
        augmented_df["selfies"] = df["selfies"]
         ...
         logger.warning("Augmented DataFrame is missing 'selfies' column after augmentation process.")
