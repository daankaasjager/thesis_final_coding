...
def evaluate(checkpoint, csv_path, prop_cols, batch_size=256, denorm=True):
    ...
    plt.tight_layout()
    fig.savefig("/scratch/s3905845/thesis_final_coding/data/kraken/metric_plots/val_mae_per_property.png", dpi=150)
    print("finished evaluation")
    return df_mae

if __name__ == "__main__":
    CHECKPOINT  = "/scratch/s3905845/thesis_final_coding/checkpoints/property_prediction.ckpt"
    CSV         = "/scratch/s3905845/thesis_final_coding/data/kraken/training_data/filtered_selfies.csv"
    PROP_COLS   = ['nbo_P', 'nmr_P', 'pyr_P', 'fmo_mu', 'vmin_r', 'volume', 'fmo_eta', 
                   'fukui_m', 'fukui_p', 'nuesp_P', 'somo_rc', 'nbo_P_rc', 'pyr_alpha', 
                   'qpole_amp', 'vbur_vbur', 'Pint_P_min', 'sterimol_L', 'sterimol_B1', 
                   'sterimol_B5', 'dipolemoment', 'efgtens_xx_P', 'efgtens_yy_P', 'nbo_bd_e_max',
                     'nbo_lp_P_occ', 'qpoletens_yy', 'E_solv_elstat', 'nbo_bds_e_avg', 'sterimol_burL',
                       'nbo_bd_occ_avg', 'sterimol_burB5', 'vbur_ovbur_min', 'vbur_qvbur_min', 
                       'nbo_bds_occ_max', 'vbur_ratio_vbur_vtot', 'mol_wt', 'sa_score']

    table = evaluate(CHECKPOINT, CSV, PROP_COLS)
    table.to_csv("/scratch/s3905845/thesis_final_coding/val_mae_per_property.csv", index=False)
    print(table)