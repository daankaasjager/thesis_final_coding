...
def train_selfies_bpe_vocab(
    ...
    if num_merges_to_do <= 0:
        logger.warning("Target max_vocab_size is not larger than initial vocab size. No merges will be performed.")
    ...
    for i in tqdm(range(num_merges_to_do), desc="Merging pairs", unit="merge", disable=not verbose):
        ...
        if not pair_counts:
            if verbose:
                logger.info(f"Merge iteration {i+1}: No more pairs found in the corpus representation.")
            break # No pairs left
        ...
        for pair, freq in sorted_pairs:
             ...
             if merged_token_candidate not in current_vocab:
                  ...
                  if freq >= min_freq_for_merge:
                      ...
                      if verbose:
                          logger.info(f"Merge iteration {i+1}: Most frequent pair {pair} frequency ({freq}) is below threshold ({min_freq_for_merge}). Stopping merges.")
        ...
        if best_pair == (None, None) or max_freq < min_freq_for_merge:
             ...
             if best_pair == (None, None) and max_freq >= min_freq_for_merge:
                  logger.info(f"Merge iteration {i+1}: All frequent pairs already resulted in existing tokens. Stopping merges.")
        ...
        if verbose:
            logger.info(f"Merge {i+1}/{num_merges_to_do}: Merging {best_pair} -> '{merged_word}' (freq: {max_freq}). New vocab size: {len(current_vocab)}")
        new_word_sequences = []
        for sequence in word_sequences:
            new_sequence = []
            j = 0
    ...
    if verbose:
        logger.info(f"BPE vocabulary training complete. Final vocabulary size: {len(current_vocab)}")
    with open(config.local_paths.selfies_ape_vocab, "w", encoding="utf-8)") as vocab_file:
